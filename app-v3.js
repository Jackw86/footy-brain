...
const SCREENS = [
  ...
  'prizes-screen','profile-screen','settings-screen',
];
...
function showScreen(id) {
  ...
  const navScreens = ['home-screen','prizes-screen','profile-screen'];
  elId('main-nav') && elId('main-nav').classList.toggle('hidden', !navScreens.includes(id));

  if (id === 'profile-screen') renderProfile && renderProfile();
  if (id === 'settings-screen') renderSettings && renderSettings();
  if (id === 'daily-screen') renderDailyChallenge && renderDailyChallenge();
  if (id === 'home-screen') renderPath && renderPath();
}

function setActiveNav(id) {
  document.querySelectorAll('#main-nav button').forEach(b => b.classList.remove('active'));
  elId(id) && elId(id).classList.add('active');
}
...
function saveNameAndShowPosition() {
  // Try all common ID variants for the name input
  const input = elId('player-name') || elId('playerName') || elId('name-input') || elId('nameInput') || elId('username');
...
(function init() {
  ...
  // Attach Enter key listener to name input — try all common ID variants
  const ni = elId('player-name') || elId('playerName') || elId('name-input') || elId('nameInput') || elId('username');
