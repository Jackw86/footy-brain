// ============================================================
//  PitchForge Academy — games.js  v2
//  All 6 mini-games. Requires app.js (addXP, showToast,
//  showScreen, S, GAME_META) and data.js to be loaded first.
// ============================================================
...
function gameOver(container, gameName, score, maxScore, xpEarned) {
  ...
  if (isHi) S.gameHighScores[gameName] = score;
  saveState();

  const pct   = Math.round(score / maxScore * 100);
  const emoji = pct === 100 ? '🏆' : pct >= 80 ? '⭐' : pct >= 50 ? '👍' : '💪';

  if (pct === 100) confetti({ particleCount: 130, spread: 70, origin: { y: 0.55 } });
  ...
  container.innerHTML = `
    <div style="text-align:center;padding:2rem 1rem;">
      ...
      <h2 style="font-family:'Bebas Neue',sans-serif;font-size:2.8rem;
          color:${pct>=50?'var(--green)':'var(--orange)'};line-height:1;">
        ${score} / ${maxScore}
      </h2>
      ${isHi ? `<div style="color:var(--orange);font-weight:700;margin:.4rem 0;">
        🏅 New high score!</div>` : `<div style="color:var(--text2);font-size:.85rem;margin:.4rem 0;">
        Best: ${prev}/${maxScore}</div>`}
      <div style="display:inline-block;background:var(--card2);padding:.5rem 1.2rem;
          border-radius:99px;margin:.75rem 0 1.25rem;font-weight:700;color:var(--green);">
        +${xpEarned} XP
      </div>
      <div style="display:flex;flex-direction:column;gap:.6rem;">
        <button class="btn btn-green" style="width:100%;"
          onclick="showGame('${gameName}')">Play Again 🔁</button>
        <button class="btn btn-gray"  style="width:100%;"
